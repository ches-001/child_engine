...
void select_move(chess::Movelist &movelist, int start_idx){
    // instead of sorting the movelist array, I opted for moving the most valuable move at a given
    // time to the left-side of the array (at start_idx), although this by itself is less efficient
    // than physically sorting the array, it is more efficient in a context where beta-cutoff occurs
    // very frequently. In otherwords the efficiency of this technique hinges on how good the move
    // scoring and static evaluation methods are, because a good move scoring technique will trigger
    // more beta cut-offs.
